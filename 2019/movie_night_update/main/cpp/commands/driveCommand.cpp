...
void driveCommand::Execute() {
  ...
  driveSystem->ArcadeDrive(0.6 * oi->logiStick->GetY(), 0.5 * oi->logiStick->GetZ(), true);
...
// Close Button 2
  ...
  if (oi->logiStick->GetRawButton(11) || oi->logiStick->GetRawButton(12)) {
    turn_speed =  prefs->GetDouble("slow_turn_speed", 1.0f);
  }
  else {
    turn_speed =  prefs->GetDouble("fast_turn_speed", 1.0f);
  }
